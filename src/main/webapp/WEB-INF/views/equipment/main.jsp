...
<sec:authorize access="isAuthenticated()">
  ...
  <form action="/equipment/report" method="post">
    <table>
      ...
      <tr>
        <th>트레이너</th>
        <td><sec:authentication property="name"/><input type="hidden" name="username" value="<sec:authentication property='name'/>"/></td>
      </tr>
      <tr>
        <th>내용</th>
...
<c:choose>
  ...
  <c:otherwise>
    ...
    <c:if test="${pager.totalPage > 1}">
      ...
      <div class="page-info">
        총 ${pager.totalCount}건 중 ${pager.curPage}/${pager.totalPage} 페이지
      </div>
    </c:if>
  </c:otherwise>
</c:choose>
