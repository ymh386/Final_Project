...
<body>
    ...
    <h2>통계 현황</h2>
    <p>전체 비품: ${totalCount}</p>
    <p>정상: ${normalCount}</p>
    <p>점검중: ${maintenanceCount}</p>
    <p>고장: ${faultCount}</p>

    <h2>검색</h2>
    <form action="/equipment/list" method="get">
        비품명: <input type="text" name="search" value="${param.search}" />
                ...
                <tbody>
                    <c:forEach items="${equipmentList}" var="equipment">
                        <tr>
                            ...
                            <td>
                                <c:choose>
                                    <c:when test="${not empty equipment.lastMaintenanceAt}">
                                        ${equipment.lastMaintenanceAt.format(
                            ...
                            <td>
                                <c:choose>
                                    <c:when test="${not empty equipment.createdAt}">
                                        ${equipment.createdAt.format(
                                            java.time.format.DateTimeFormatter.ofPattern("yyyy-MM-dd")
                                        )}
                                    </c:when>
                                    <c:otherwise>-</c:otherwise>
